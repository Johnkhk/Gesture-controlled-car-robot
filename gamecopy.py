...
while True:
    ...
    if curr_time - last_recorded_time >= 2.0:
        if user_move_name == "scissors":
            client.publish('raspberry/topic', payload="forward", qos=0, retain=False)
            print(f"send forward to raspberry/topic")
        elif user_move_name == "rock":
            client.publish('raspberry/topic', payload="left", qos=0, retain=False)
            print(f"send left to raspberry/topic")
        elif user_move_name == "paper":
            client.publish('raspberry/topic', payload="right", qos=0, retain=False)
            print(f"send right to raspberry/topic")
        else:
            client.publish('raspberry/topic', payload="stay", qos=0, retain=False)
            print(f"send stay to raspberry/topic")
